...
const NotificationItem: React.FC<{ notification: Notification; onToggleRead: (id: string) => void; }> = ({ notification, onToggleRead }) => {
    ...
    return (
        <div className={`p-3 md:p-4 flex items-start gap-4 border-l-4 ${notification.read ? 'border-transparent' : 'border-primary bg-primary/5 dark:bg-primary/10'}`}>
            <div className="flex-shrink-0 w-3 h-3 bg-primary rounded-full mt-1.5 opacity-50"></div>
            <div className="flex-1">
                <div className="flex justify-between items-center">
                    <p className="font-semibold text-gray-900 dark:text-on-surface text-sm md:text-base">{notification.title}</p>
                    <span className="text-xs text-gray-500 dark:text-on-surface-variant flex-shrink-0 ml-2">{timeAgo(notification.timestamp)}</span>
                </div>
                <p className="text-sm text-gray-600 dark:text-on-surface-variant mt-1">{notification.description}</p>
    ...
    <div className="p-4 md:p-6 lg:p-8 space-y-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-on-surface">Notifications</h1>
      ...
      <div className="bg-white dark:bg-surface rounded-lg shadow-lg overflow-hidden">
